...
// The below is required so that strings can be split among multiple lines.
...
var parseText = function(text) {
	...
	if( afterLastColon.indexOf('-') == -1 ) {
		...
		if( addressText === undefined ) {
			...
			if( colonParts !== undefined && colonParts.size() > 5) {
				addressText = colonParts[6];
			}
		}
	}
    var addressDescr;
    ...
    if( addressText === undefined ) {
    	console.log('Unable to parse addressText from dispatch: ' + text);
    	return "";
    }
...
// All set to lower case when evaluated against the below
var absoluteValues = {
	'rt. 202/206' : GREEN_KNOLL,
	'u s hwy no 202 hwy': GREEN_KNOLL,
	'hwy nos 202 and 206 hwy': GREEN_KNOLL,
	'rt. 22' : GREEN_KNOLL,
...
function normalizeStreetName(street) {
	...
	return trim(street.replace(/ /g, '_').toLowerCase());
}
...
var dropNSEW = function(streetName) {
	var lastUnder = streetName.lastIndexOf('_');
	if( lastUnder < 0 ) {
		return streetName;
	}
	if( streetName.endsWith('_north') || streetName.endsWith("_n") || 
		streetName.endsWith("_south") || streetName.endsWith("_s") ||
		streetName.endsWith("_east")  || streetName.endsWith("_e") ||
		streetName.endsWith("_west")  || streetName.endsWith("_w") ) {
		return replaceStreetAbbr(streetName.substring(0, lastUnder));
	} else {
		return streetName;
	}
};
...
function findTerritory(dispatch) {
	...
	var streetNum = trim(streetAddress).split(' ')[0];
	var streetName = trim(streetAddress).substring(streetNum.length+1);

	if( !isNumber(streetNum) ) {
		streetNum = 0;
		streetName = trim(streetAddress);
